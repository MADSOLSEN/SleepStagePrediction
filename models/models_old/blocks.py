...
def dense_layer_1D(x, width, dropout_rate, BN_momentum=0.99):
...
def conv_layer_1D(y, nb_channels_out, kernel_size=3, strides=1, dilation_rate=1, cardinality=1):
    if cardinality == 1:
        try:
            return layers.Conv1D(nb_channels_out,
    ...
    assert not nb_channels_out % cardinality
    _d_out = nb_channels_out // cardinality
...
def conv_layer_2D(y, nb_channels_out, kernel_size, strides=(1, 1), dilation_rate=(1, 1), cardinality=1):

    if cardinality == 1:
        return layers.Conv2D(nb_channels_out,
                             kernel_size=kernel_size,
                             strides=strides,
                             padding='same',
                             dilation_rate=dilation_rate,
...
def residual_block_1D(y, nb_channels_in, nb_channels_out, kernel_size, _strides=1, _project_shortcut=False,
                   bottle_neck=False):
    """
    ...
    if bottle_neck:
        ...
        y = conv_layer_1D(y, nb_channels_out=nb_channels_in, kernel_size=kernel_size, strides=1)
        y = add_common_layers_1D(y)
        ...
        y = conv_layer_1D(y, nb_channels_out=nb_channels_out, kernel_size=kernel_size, strides=1, dilation_rate=1)
        y = add_common_layers_1D(y)
...
def residual_block_2D(y, nb_channels_in, nb_channels_out, kernel_size, _strides=(1, 1), _project_shortcut=False, bottle_neck=False):
    ...
    if bottle_neck:
        ...
        # ResNeXt (identical to ResNet when `cardinality` == 1)
        y = conv_layer_2D(y, num_channels=nb_channels_in, kernel_size=kernel_size, strides=1)
        y = add_common_layers_2D(y)
        ...
        y = conv_layer_2D(y, num_channels=nb_channels_out, kernel_size=kernel_size, strides=1, dilation_rate=(1, 1))
        y = add_common_layers_2D(y)
...
def wavenet_residual_block_1D(y, nb_channels_out, kernel_size, dilation_rate):
    # Gated activation.

    l_sigmoid_conv1d = layers.Conv1D(nb_channels_out,
                                     kernel_size,
                                     dilation_rate=dilation_rate,
                                     padding="same",
                                     activation="sigmoid")(y)
    l_tanh_conv1d = layers.Conv1D(nb_channels_out,
                                  kernel_size,
                                  dilation_rate=dilation_rate,
                                  padding="same",
                                  activation="tanh")(y)
...
def wavenet_residual_block_2D(l_input, nb_channels_out, kernel_size, dilation_rate):
    # Gated activation.
    l_sigmoid_conv1d = layers.Conv2D(nb_channels_out,
                                     kernel_size=kernel_size,
                                     dilation_rate=dilation_rate,
                                     padding="same",
                                     activation="sigmoid")(l_input)
    l_tanh_conv1d = layers.Conv2D(nb_channels_out,
                                  kernel_size=kernel_size,
                                  dilation_rate=dilation_rate,
                                  padding="same",

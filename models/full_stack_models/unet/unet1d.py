...
def unet(input_shape,
    ...
    for i in reversed(range(depth)):
        features = features // 2
        x = layers.UpSampling2D(size=(max_pool_size, 1), data_format=data_format)(x)
        x = layers.Conv2D(features, (2, 1), activation='relu', padding='same', data_format=data_format)(x)

...
def r2_unet(input_shape,
    ...
    for i in range(depth):
        x = rec_res_block(x, features, data_format=data_format, batch_normalization=batch_normalization)
    ...
    for i in reversed(range(depth)):
        features = features // 2
        x = up_and_concate(x, skips[i], upsample_size=max_pool_size, data_format=data_format)
        x = rec_res_block(x, features, data_format=data_format, batch_normalization=batch_normalization)
        x = layers.Dropout(dropout)(x)
